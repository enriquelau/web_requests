...
# url and quantity should be command line arguments
...
def add_to_cart(pid):
    ...
    response = session.post('https://www.federalpremium.com/on/demandware.store/Sites-VistaFederal-Site/default/Cart-AddProduct',
...
def get_state_restriction(shipping_info):
    ...
    url = "https://www.federalpremium.com/on/demandware.store/Sites-VistaFederal-Site/default/Cart-CheckShippingRestrictions" \
...
def verify_state(shipmentUUID):
    url = "https://www.federalpremium.com/on/demandware.store/Sites-VistaFederal-Site/default/CheckoutShippingServices-UpdateShippingMethodsList?firstName=&lastName=&address1=&address2=&city=&postalCode=&stateCode=&countryCode=&phone=&shipmentUUID=" + shipmentUUID
...
def verify_address(shipmentUUID):
    ...
    url = "https://www.federalpremium.com/on/demandware.store/Sites-VistaFederal-Site/default/CheckoutShippingServices-UpdateShippingMethodsList?firstName=&lastName=&address1=" + street + "&address2="+street2+"&city=&postalCode=&stateCode=TX&countryCode=US&phone=&shipmentUUID=" + shipmentUUID
    response = session.post(url)
    return response.json()["shippingForm"]["valid"]


def get_csrf_token():
    response = session.get("https://www.federalpremium.com/on/demandware.store/Sites-VistaFederal-Site/default/Checkout-Begin")
