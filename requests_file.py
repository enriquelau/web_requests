...
def verify_address(shipmentUUID):
    ...
    street = environ.get("STREET_DOUBLE_SPACED")

    street = "9958++Twin++Shores++Dr"
    url = "https://www.speer.com/on/demandware.store/Sites-VistaSpeer-Site/default/CheckoutShippingServices-UpdateShippingMethodsList?firstName=&lastName=&address1=" + street + "&address2=&city=&postalCode=&stateCode=TX&countryCode=US&phone=&shipmentUUID=" + shipmentUUID
    response = session.post(url)
...
def build_shipping_info():
    ...
    shipping_info = {"first_name": environ.get("FIRST_NAME"), "last_name": environ.get("LAST_NAME"),
                     "address": environ.get("STREET"), "state": environ.get("STATE"),
...
def submit_payment(shipping_info, payment_info, csrf_token):
    ...
    query = "?addressSelector=manual-entry&" \
            ...
            "&dwfrm_billing_addressFields_address2=&dwfrm_billing_addressFields_country=US" \
            "&dwfrm_billing_addressFields_states_stateCode=" + shipping_info["state"] + \
            "&dwfrm_billing_addressFields_city=" + shipping_info["city"] + \
            "&dwfrm_billing_addressFields_postalCode=77318" \
            "&csrf_token=" + csrf_token + \
